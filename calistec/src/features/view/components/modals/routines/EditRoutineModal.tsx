...
export default function UpdateRoutineModal({
    ...
    // Valores iniciales
    const [form, setForm] = useState<RoutineFormState>(() => createRoutineFormStateFromRoutine(existingRoutine));

    // Información selecta
    const [selectedExerciseByRow, setSelectedExerciseByRow] = 
    useState<Record<string, {ejercicioId: string, title: string, focus: string, level: string}>>(() => {
        const map: Record<string, {ejercicioId: string, title: string, focus: string, level: string}> = {};
        const found = existingRoutineExercise.find(
            (item) => item.exercise_info?.title
        );
        if (found){
            map[`ejercicio`] = {
                ejercicioId: String(found.ejercicio_id),
                title: String(found.exercise_info!.title),
                focus: String(found.exercise_info!.focus),
                level: String(found.exercise_info!.level)
            }
        }
        return map
    })
    ...
    const [selectedExerciseTable, setSelectedExerciseTable] = useState<SelectedExerciseItem[]>(() => 
        existingRoutineExercise
            .filter((item) => item.exercise_info?.title)
            .map((item) => ({
                row: "ejercicio",
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
            <div className="max-h-[95vh] w-full max-w-4xl overflow-hidden rounded-[28px] 
                ...
                <div className="flex items-center justify-between border-b border-emerald-400/10 px-6 py-5">
                    <div>
                        <h2 className="mt-1 text-2xl font-black text-white">Editar rutina</h2>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
