                    ...
                    <table className="min-w-full border-separate border-spacing-0">
                        <thead>
                            <tr className="table-head text-left">
                                {Exercise_TABLE_HEADERS.map((header) => (
                                    <th
                                        key={header}
                            ...
                            {exercises.length > 0 ? (
                                exercises.map((exercise) => (
                                    <tr key={exercise.id} className="table-row bg-transparent align-top text-sm text-emerald-50/90">
                                        <td className="px-4 py-5 font-semibold text-white">{exercise.title}</td>
                                        <td className="px-4 py-5">
                                            <div className="flex flex-wrap gap-2">
                                                {String(exercise.focus)
                                                    .split(/\s+/)
                                                    .filter(Boolean)
                                                    .map((item) => (
                                        ...
                                        <td className="px-4 py-5">
                                            <span className={`badge ${getLevelBadgeClass(String(exercise.level))}`}>{exercise.level}</span>
                                        </td>
                                        <td className="px-4 py-5">
                                ...
                                <tr className="bg-transparent">
                                    <td colSpan={Exercise_TABLE_HEADERS.length} className="px-4 py-12 text-center text-emerald-100/55">
