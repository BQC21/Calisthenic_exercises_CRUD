...
import {
    ...
    getExeFilterOptions,
    updateExe,
} from "@/features/controllers/services/ExeQueries";
import type { 
    Exercise,
    ExerciseFormData,
    UseExerciseResult,
...
} from "@/lib/types/services/exe-types";

import { createClient } from "@/lib/supabase/client";

const supabase = createClient();

export function useExercise(): UseExerciseResult {
    ...
    useEffect(() => {
        const channelName = `ejercicios-realtime-${Date.now()}-${Math.random().toString(36).slice(2)}`; // previene colision de suscripciones
