...
export function useRoutine(): UseExercise_RoutineResult {
    ...
    useEffect(() => {
        const channelName = `ejercicio-rutina-realtime-${Date.now()}-${Math.random().toString(36).slice(2)}`; // previene colision de suscripciones
        const channel = supabase
            .channel(channelName)
            .on(
                "postgres_changes",
                { event: "INSERT", schema: "public", table: "ejercicio-rutina" },
                () => {
                    void fetchRoutines_exercises();
                }
            )
            .on(
                "postgres_changes",
                { event: "UPDATE", schema: "public", table: "ejercicio-rutina" },
                () => {
                    void fetchRoutines_exercises();
                }
            )
            .on(
                "postgres_changes",
                { event: "DELETE", schema: "public", table: "ejercicio-rutina" },
                () => {
                    void fetchRoutines_exercises();
                }
            )
